...
def inference(args):
    ...
    original_model_path = f'{args.stylegan_dir}/stylegan2-ffhq-config-f.pt'
    ...
    original = Generator(1024, latent_dim, 8, 2).to(device)
    original.load_state_dict(ckpt["g"], strict=False)
    ...
    my_toonify = original(my_w, input_is_latent=True)
    if args.time_stamp:
